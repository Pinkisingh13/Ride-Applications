import 'dart:convert';
import 'dart:io';
import 'package:driver/features/auth/view/login/login.dart';
...
// Varibles
  ...
  Future<void> signup(UserSignup user) async {
    try {
      http.Response response = await http.post(
        Uri.parse('http://192.168.1.6:3000/driver/register'),
        headers: {
  ...
  Future<void> login(LoginData user) async {
    try {
      http.Response response = await http.post(
        Uri.parse('http://192.168.1.6:3000/driver/login'),
        headers: {
          HttpHeaders.contentTypeHeader: "application/json",
        },
        body: jsonEncode(user.toJson()),
